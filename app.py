...
def products():
    allproduct = Product.query.all()
    return render_template("products.html", products=allproduct, cart=session['cart_size'],mob=session['mobile_no'])
    ...
    if request.method == "POST":
        ...
        try:
            ...
            allproduct = Product.query.filter(
                or_(Product.category.like('%' + session['search'] + '%'),
                    Product.product_type.like('%' + session['search'] + '%'),
                    Product.product_name.like('%' + session['search'] + '%'))).all()
...
def purchase():
    try:
        ...
        return render_template("products.html", products=allproduct,
                               cart=session['cart_size'], success="Order Placed Successfully",mob=session['mobile_no'])
...
def add_product():
    ...
    if request.method == "POST":
        try:
            ...
            productType = request.form.get('productType').strip()
            ...
            result = Product(category=category, product_type=productType, product_name=productName,
                             size=size, rate=rate, image=image.filename)
            db.session.add(result)
            db.session.commit()
